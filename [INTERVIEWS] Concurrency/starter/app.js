...
async function userMap() {
  const userData = (await fetchFollowerIds()).map((id) => fetchUserData(id));
  const data = await Promise.all(userData);
  const profileEl = document.querySelector(".followers");
  profileEl.innerHTML = data.map((user) => setHtml(user));
}
